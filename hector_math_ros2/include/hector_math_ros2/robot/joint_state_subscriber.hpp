...
public:
      ...
      : model_( std::move( model ) ), clock_( node.get_clock() )
  {
    ...
    sub_ = node.create_subscription<sensor_msgs::msg::JointState>(
        topic, 10, &JointStateSubscriber<Scalar>::onJointStateMessage, this );
  }
   ...
   */
  bool waitForFullState( const rclcpp::Duration &duration = rclcpp::Duration( 0 ))
  {
    if ( positions_.empty() )
      return true;
    ...
    while ( duration.isZero() || ( clock_->now() - start_time ) < duration ) {
...
private:
  void onJointStateMessage( const sensor_msgs::msg::JointState::ConstPtr &msg )
  {
    {
